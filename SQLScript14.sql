/**********************************************************************************************************************
Stored Procedure Name: [dwhcda].[SP_9_header_item_consolidation_maxcols]
...
CREATE PROCEDURE [dwhcda].[SP_9_header_item_consolidation_maxcols] 
...
DECLARE @lookup_cols_list  VARCHAR(MAX)
...
UNION ALL
'+ @mod_lookup_cols_list+' ) EXCEPT ('+@mod_input_pk_list +') ) b OPTION (LABEL = ''SP_9_header_item_consolidation_maxcols_1'')'
...
DECLARE @tsql8 nvarchar(max) = 'select @pk_keys = 
   STRING_AGG(name,'+''','''+') from (
select name from #cttae1 INTERSECT select name from #cttae2 ) b OPTION (LABEL = ''SP_9_header_item_consolidation_maxcols_2'')'
...
select name from #cttae1
UNION ALL
'+ @mod_lookup_cols_list+' ) EXCEPT ('+@mod_input_pk_list +') ) b OPTION (LABEL = ''SP_9_header_item_consolidation_maxcols_3'')'
...
select name from #cttae1
UNION ALL
'+ @mod_lookup_cols_list+' ) INTERSECT ('+@mod_input_pk_list +') ) b OPTION (LABEL = ''SP_9_header_item_consolidation_maxcols_4'')'
...
---print 'join_pk_col_list '+@join_pk_col_list 
	...
	IF OBJECT_ID('tempdb..#dst_to_trn', 'U') IS NOT NULL
		DROP TABLE #dst_to_trn;

	Declare @tsql11 nvarchar(max) = 
	'Select * into #dst_to_trn FROM ' +  @Parameter8 + '.' + @Parameter2 + ' Where 1=2;'
	...
	/*@Parameter7 is the joining table column list */
	
	Declare @tsql12 nvarchar(max) = 
	'Insert Into #dst_to_trn (' + @source_col_list + ',' + @Parameter7 + ') ' + @sub_table + ' OPTION (LABEL = ''SP_9_header_item_consolidation_maxcols_5'');'
	...
	'DELETE B 
	 FROM ' + @Parameter8 + '.' + @Parameter2 + ' AS A 
	INNER JOIN #dst_to_trn AS B 
	...
	'DELETE B
	 FROM #dst_to_trn as A 
	INNER JOIN ' + @Parameter8 + '.' + @Parameter2 + ' AS B
	...
	/*@Parameter7 contains joining tables column list */
	
	Declare @tsql15 nvarchar(max) = 
	'INSERT INTO ' + @Parameter8 + '.' + @Parameter2 + '(' + @target_col_list + 
	') (Select ' + @in_source_col_list + ',' + @Parameter7 + ' FROM #dst_to_trn as B ) OPTION (LABEL = ''SP_9_header_item_consolidation_maxcols_6'');'
...
values ('+@in_source_col_list + ',' + @sep_lookup_cols_list +') OPTION (LABEL = ''SP_9_header_item_consolidation_maxcols_5'');'
